...
function parseGo(command) {
  ...
  if (go[1] == "wtime" && engine.getSide() == engine.COLOR["WHITE"]) {
    timing.time = parseInt(go[2]);
  }
  if (go[3] == "btime" && engine.getSide() == engine.COLOR["BLACK"]) {
    timing.time = parseInt(go[4]);
  }
  if (go[5] == "winc" && engine.getSide() == engine.COLOR["WHITE"]) {
    inc = parseInt(go[6]);
  }
  if (go[7] == "binc" && engine.getSide() == engine.COLOR["BLACK"]) {
    inc = parseInt(go[8]);
  }
  if (go[9] == "movestogo") {
    movestogo = parseInt(go[10]);
  }
  if (go[1] == "movetime") {
    movetime = parseInt(go[2]);
  }
  if (go[1] == "depth") {
    depth = parseInt(go[2]);
  }
...
function parsePosition(command) {
  ...
  else if (position[1] == "fen")
    engine.setBoard(command.split("position fen ")[1]);
